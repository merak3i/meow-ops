...
### Added
- **Local Usage Receipt v1**: sanitized JSONL receipts from other computers can be copied into `MEOW_LOCAL_USAGE_IMPORTS` and aggregated on the primary dashboard by pseudonymous machine, harness, provider, and model. Hermes `session_model_usage` is the official read-only adapter. Other harnesses use an explicit receipt writer. Ollama and LM Studio model lists are not usage. Session assignment requires an exact stable identifier. Unmatched receipts stay as aggregates. Unknown cost stays unavailable, not `$0`.
...
- Env: `MEOW_TZ`, `MEOW_NO_SNIPPETS`, `ANTIGRAVITY_DIR`, `MEOW_DASHBOARD_ORIGIN`, `CURSOR_PROJECTS_DIR`, `CURSOR_ADMIN_API_KEY`, `MEOW_LOCAL_USAGE_IMPORTS`.

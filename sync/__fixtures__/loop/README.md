...
3. **Must-fail entries are mandatory.** `golden-proposals.json` carries
   negative cases (`expect_fail: "<rule-id>"`) so the approval gate and
   redaction rules cannot regress silently. If you add a validator rule, add
